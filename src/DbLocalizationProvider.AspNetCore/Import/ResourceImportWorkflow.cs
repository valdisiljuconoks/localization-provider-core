    ...
    public class ResourceImportWorkflow
    {
        ...
        public IEnumerable<string> ImportChanges(ICollection<DetectedImportChange> changes)
        {
            ...
            foreach (var insert in changes.Where(c => c.ChangeType == ChangeType.Insert))
            {
                ...
                insert.ImportingResource.Translations.ForEach(t => t.Value = t.Value ?? "");
            ...
            foreach (var update in changes.Where(c => c.ChangeType == ChangeType.Update))
            {
                ...
                if (existingResource == null)
                {
                    ...
                    update.ImportingResource.IsModified = false;
                    update.ImportingResource.Translations.ForEach(t => t.Language = t.Language ?? "");
                    update.ImportingResource.Translations.ForEach(t => t.Value = t.Value ?? "");
                ...
                {
                    _commandExecutor.Execute(new CreateOrUpdateTranslation.Command(existingResource.ResourceKey, new CultureInfo(translation.Language), translation.Value));
                }
